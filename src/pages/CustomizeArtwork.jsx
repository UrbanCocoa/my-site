...
export default function CustomizeArtwork() {
  ...
  useEffect(() => {
    ...
    if(rates[currency]) setPrice((basePrice * rates[currency]).toFixed(2));
    else setPrice(basePrice);
  ...
  const handleAddToCart = () => {
    ...
    if(uploadedImages.length < numProjects){
      alert(`⚠️ Please upload ${numProjects} image(s).`);
      return;
    }
      ...
      <main className="relative z-[100] flex justify-center items-start p-10 flex-1 gap-8">
        <div className="w-full max-w-lg p-8 flex flex-col gap-6 rounded-xl shadow-lg bg-accent2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select value={numProjects} onChange={(e)=>setNumProjects(parseInt(e.target.value))}
          ...
          <div className="flex flex-col gap-2">
            ...
            <input type="file" multiple accept="image/*" onChange={handleFileChange} className="p-2 rounded-lg bg-accent3 text-accent5 shadow-md"/>
            {uploadedImages.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-2">
                {uploadedImages.map((file, idx)=>(
                  <img key={idx} src={URL.createObjectURL(file)} alt={`Preview ${idx}`} className="w-24 h-24 object-contain rounded-lg border border-accent3"/>
                ))}
              </div>
            )}
          </div>
          ...
          <button onClick={handleAddToCart} type="button" className="px-6 py-4 mt-4 text-lg font-bold rounded-lg shadow-md text-accent5 bg-accent3 hover:bg-darker transition">
            Add to Cart
          </button>
